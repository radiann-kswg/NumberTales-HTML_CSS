  ...
  startTags: function (array_tag, cb) {
    ...
    let next_tag = () => {
        ...
        if (this.current_tags.length == this.cntTag) {
          this.current_cb && this.current_cb();
      ...
      post_tag = () => {
        ...
        tobj && tobj.pm.cond && 0 == TYRANO.kag.ftag.checkCond(tobj)
          ? next_tag()
...
tyrano.plugin.kag.tag.text = {
  ...
  showMessage: function (message_str, is_vertical) {
    ...
    this.kag.stat.charas[chara_name] &&
      "" != this.kag.stat.charas[chara_name].jname &&
  ...
  adjustCharaFukiSize: function (j_msg_inner, chara_name, chara_obj) {
    ...
    if ("true" != this.kag.stat.vertical && "" == chara_fuki.fix_width) {
  ...
  addOneChar: function (
    ...
    this.kag.tmp.popopo.key &&
      this.kag.tmp.popopo.player.play(
        j_char_span_children.eq(char_index).text()
      );
    if (next_char_index < j_char_span_children.length)
      ...
      this.kag.tmp.popopo.key && this.kag.tmp.popopo.player.stop();
      $.setTimeout(() => {
